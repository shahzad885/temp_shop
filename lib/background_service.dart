...
void onBackgroundServiceStart(ServiceInstance service) async {
  ...
  // ── Seed knownFiles from BOTH filesystem AND Hive ────────────────────────
...
Future<void> _checkForNewFiles(
  ...
  for (final path in newFiles) {
    ...
    // Show floating overlay via notification (overlay handled natively)
...
Future<void> _showExpiryNotification(String screenshotId) async {
  ...
  const androidDetails = AndroidNotificationDetails(
    ...
    autoCancel: true,
    // Show as a heads-up / bubble notification so it appears as overlay
